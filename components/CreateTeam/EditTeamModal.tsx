...
export default function EditTeamModal({ visible, onClose, onUpdateTeam, teamData }: EditTeamModalProps) {
  // Animation values
  ...
  // Initialize state with existing team data when modal opens
  useEffect(() => {
    if (visible && teamData) {
      setTeamName(teamData.name || '');
      setTeamSize(teamData.teamSize || 11);
      setSelectedFormation(teamData.formation || null);
      setPlayers(teamData.players || []);
      
      // Handle custom team size
      if (teamData.teamSize && ![5, 7, 11].includes(teamData.teamSize)) {
        setIsCustomSize(true);
        setCustomSize(teamData.teamSize.toString());
      } else {
  ...
  useEffect(() => {
    if (selectedFormation && teamData) {
      // Only reset if the team size actually changed from the original
      if (teamSize !== teamData.teamSize) {
        ...
        <View style={styles.backdropContainer}>
          <Animated.View style={[styles.modalContainer, animatedModalStyle]}>
            ...
            <View style={styles.footer}>
              ...
              <TouchableOpacity
                style={[styles.updateButton, !canUpdate() && styles.disabledButton]}
